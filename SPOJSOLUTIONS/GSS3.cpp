...
void update(long long l,long long r,long long index,long long y,long long node,vector<tuple<long long ,long long,long long,long long >>&s,vector<long long>&a)
{ if(index>=l&&l==r)
  {a[l]=y;
   s[node]=make_tuple(y,y,y,y);
  return;}
  long long mid=(l+r)>>1;
  if(index>=l&&index<=mid)
    update(l,mid,index,y,2*node+1,s,a);
  else
    update(mid+1,r,index,y,2*node+2,s,a);
